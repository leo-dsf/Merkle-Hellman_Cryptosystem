...
char HorowitzSahni(int n, const integer_t p[n], integer_t desired_sum, int result[])
{
    ...
    duplicated_s1 = (integer_t *)calloc(total1 * sizeof(integer_t));
    duplicated_s2 = (integer_t *)calloc(total2 * sizeof(integer_t));
    ...
    integer_t size1 = total1;
    integer_t size2 = total2;
    ...
    while (i < total1)
    {
        ...
        if (i < total2 && sum2 < desired_sum)
        {
            printf("sum2 = %llu", sum2);
            s2[i] = sum2;
